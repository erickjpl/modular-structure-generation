...
class InfrastructureLayerGenerator(IFileGenerator):
  def generate(self, context: dict):
    ...
    if InfrastructureOption.PERSISTENCE in options:
      self._generate_persistence(module_name, app_name, context.get("persistence_options", []))

    # Implementar otros métodos para api, consumers, etc.

  def _generate_persistence(self, module_name: str, app_name: str, persistence_options: list[PersistenceOption]):
