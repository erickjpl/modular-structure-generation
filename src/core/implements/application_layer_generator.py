...
class ApplicationLayerGenerator(InterfaceFileGenerator):
  def generate(self, context: dict):
    ...
    if ApplicationOption.COMMANDS in options:
      self._generate_commands(module_name, app_name, context.get("command_options", []), attributes)

    # Implementar otros métodos para queries, handlers, etc.

  def _generate_commands(
    self, module_name: str, app_name: str, command_options: list[CommandOption], attributes: list[tuple[str, str]]
  ):
