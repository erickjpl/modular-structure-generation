...
from src.orders.application.dtos.order_dto import OrderDTO, OrderItemDTO
...
# Fixtures for mocking (reused from test_order_list_query_handlers.py)
@pytest.fixture
...
def test_read_order_query_handler_subscribed_to():
    handler = ReadOrderQueryHandler(repository=Mock())
    assert handler.subscribed_to() == ReadOrderQuery

def test_read_order_query_handler_handle_found(mock_order_repository, mock_order):
    mock_order_repository.get_by_id.return_value = mock_order
    handler = ReadOrderQueryHandler(repository=mock_order_repository)
    query = ReadOrderQuery(pk="order123")

    result = handler.handle(query)

    mock_order_repository.get_by_id.assert_called_once_with(pk="order123")
    assert isinstance(result, OrderDTO)
    assert result.id == mock_order.id.value
    assert result.order_number == mock_order.order_number

def test_read_order_query_handler_handle_not_found(mock_order_repository):
    mock_order_repository.get_by_id.return_value = None
    handler = ReadOrderQueryHandler(repository=mock_order_repository)
    query = ReadOrderQuery(pk="non_existent_order")

    result = handler.handle(query)

    mock_order_repository.get_by_id.assert_called_once_with(pk="non_existent_order")
